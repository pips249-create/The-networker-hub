...
| **Claim confirmed email** | Everyone who claims via the invite modal | Resend template `organiser_claim_confirmed` — badge notice + CTA to add logo/website on `/organiser/group-edit` (reply-to Catherine) |
...
- Claim confirmation email: `api/_lib/organiser-claim-confirmed-emails.js` + `email-templates/organiser-claim-confirmed.html` (migration `242_organiser_claim_confirmed_email.sql`)

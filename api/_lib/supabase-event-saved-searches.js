...
async function listEventSavedSearches(session) {
...
async function createEventSavedSearch(session, input) {
...
async function deleteEventSavedSearch(session, searchId) {
  if (!isSupabaseConfigured()) throw new Error('supabase_not_configured');
  const sb = getSupabaseAdmin();
  const attendeeId = await resolveAttendeeId(sb, session);

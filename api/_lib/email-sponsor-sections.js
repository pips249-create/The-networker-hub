...
async function resolveOpportunitySponsorBlock(sb) {
  const block = await fetchSponsorBlockForSlot(sb, OPPORTUNITIES_SPONSOR_SLOT);
  if (block && block.include_in_emails !== false && isRenderableEmailSponsor(block)) {
    return block;
  }
  return null;
}
...
async function getEmailSponsorVars(slug) {
  ...
  try {
    ...
    if (slug === 'organiser_launch_invite' || slug === 'organiser_claim_invite' || slug === 'opportunity_claim_invite' || slug === 'affiliate_claim_invite' || slug === 'franchise_claim_invite' || slug === 'franchise_claim_invite_followup' || slug === 'distributorship_claim_invite') {

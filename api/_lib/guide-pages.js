...
const GUIDE_PAGES = {
  ...
  'list-a-conference-or-exhibition': {
    ...
    steps: [
      ...
      { id: 'tickets', name: 'Set up tickets (free or paid)', text: 'Add delegate ticket tiers (early bird, standard, VIP, or free registration). Use the same price on every date if you want attendees to book all remaining dates in one checkout. For a single admission price covering every day without per-day booking, sell tickets on the first date only and state in the description that the ticket includes all days. Paid tickets require Connect Stripe under Revenue.' },

...
function mergeEmailPreviewVariables(slug, extraVars, siteUrl) {
  ...
  if (slug === 'opportunity_listing_pending_review') {
    vars.opportunity_edit_url = site + '/organiser/opportunity-edit?id=preview-opportunity';
  }

  if (slug === 'opportunity_listing_approved_pay') {
    vars.pay_url = site + '/organiser/opportunity-edit?id=preview-opportunity&checkout=start';
  ...
  if (slug === 'opportunity_page_partner_payment_welcome') {
    vars.contact_name = vars.contact_name || 'Alex';
    vars.advertising_url = vars.advertising_url || site + '/advertising#ad-pkg-opportunities-mini';
    vars.creative_email = vars.creative_email || supportEmail();
    vars.monthly_note =
      vars.monthly_note ||
  ...
  if (slug === 'opportunity_claim_invite') {
    const franchiseEmailFooterLogoDataUri = require('./franchise-email-footer-logo-datauri');
    vars.claim_url =
      vars.claim_url ||
      opportunityPublicUrl({ slug: 'marketing-agency-partnership', id: opportunityId }, site) +
        '?email=alex@example.com&intent=opportunity-claim&auth=register';
    vars.opportunity_title = vars.opportunity_title || 'Marketing agency partnership';
  ...
  if (slug === 'affiliate_claim_invite') {
    vars.claim_url =
      vars.claim_url ||
      opportunityPublicUrl({ slug: 'asos-affiliate-programme', id: opportunityId }, site) +
        '?email=affiliates@example.com&intent=opportunity-claim&auth=register';
    vars.opportunity_title = vars.opportunity_title || 'ASOS Affiliate Programme';
    vars.owner_name = vars.owner_name || vars.opportunity_title;
  }
  ...
  if (slug === 'distributorship_claim_invite') {
    const franchiseEmailFooterLogoDataUri = require('./franchise-email-footer-logo-datauri');
    vars.claim_url =
      vars.claim_url ||
      opportunityPublicUrl({ slug: 'premium-coffee-distributorship', id: opportunityId }, site) +
        '?email=sales@example.com&intent=opportunity-claim&auth=register';
    vars.opportunity_title = vars.opportunity_title || 'Premium coffee capsule distributorship';
    vars.owner_name = vars.owner_name || vars.opportunity_title;
    vars.logo_footer_light_url = franchiseEmailFooterLogoDataUri;
    vars.support_email = vars.support_email || 'hi@thenetworkeruk.com';
  }
  ...
  if (
    ...
    if (slug === 'organiser_call_followup') {
      vars.book_call_url =
        vars.book_call_url || 'https://www.thenetworkeruk.com/contact';
    }
  }

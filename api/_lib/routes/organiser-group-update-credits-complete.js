...
const { handleGroupUpdateCreditsCheckout } = require('../group-update-credits');
const { getAllowance } = require('../organiser-group-updates');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const result = await handleGroupUpdateCreditsCheckout(session);
    const allowance = await getAllowance(organiserId).catch(() => null);
    return json(res, 200, { ok: true, result, allowance });
  } catch (e) {
    return jsonPublicError(res, json, e, { code: 'credits_complete_failed', logLabel: '[organiser-group-update-credits-complete]' });
  }
};

...
const { getCreditPack, listCreditPacks, normalizePackId } = require('../connections-credits');
...
/** Start Stripe Checkout for extra Attendee round-up send credits. */
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const successUrl =
      base +
      '/organiser/?connections_credits_session={CHECKOUT_SESSION_ID}#attendee-email';
    const cancelUrl = base + '/organiser/#attendee-email';

    const session = await createConnectionsCreditsCheckoutSession({
      organiserId,
      packId: pack.id,

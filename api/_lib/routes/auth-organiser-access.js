...
module.exports = async function handler(req, res) {
    ...
    try {
      ...
      return json(res, 200, {
        ...
        message:
          ...
          session.email +
          '.',
        redirect: '/organiser/verify-email',
      });
    } catch (e) {
  ...
  if (action === 'hide-ui') {
    ...
    try {
      ...
      return json(res, 200, {
        ...
        ...status,
        redirect: '/account/',
        message: 'Organiser workspace hidden. You can still buy tickets and manage bookings in My account.',
      });
    } catch (e) {
  ...
  try {
    ...
    const status = await getOrganiserAccessStatus(session);
    const redirect = !before.organiserEmailVerified ? '/organiser/verify-email' : '/organiser/';

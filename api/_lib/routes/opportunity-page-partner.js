/**
 * Opportunity Page Partner — availability + self-serve checkout.
 * Mounted at GET/POST /api/opportunity-page-partner via cms-block rewrite.
 */
...
} = require('../opportunity-page-partner-subscriptions');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const availability = await getOpportunityPagePartnerAvailability(sb);
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'verify') {
        ...
        const finalized = await handleOpportunityPagePartnerCheckoutCompleted(session);
        const email = String(
        ...
        return res.status(200).json({
          ...
          email,
          sessionId,
          reserved: finalized?.reserved || null,
        });
      }
    ...
    if (req.method === 'POST') {
      ...
      const validation = validateOpportunityPagePartnerCheckout(availability, term, 1);
      if (!validation.ok) {
        return res.status(409).json({
          ok: false,
          error: validation.error,
          message: validation.message || 'No slots available',
          available: availability.available,
        });
      }

      const base = siteBaseUrl();
      const session = await createOpportunityPagePartnerCheckoutSession({
        email,
        termMonths: term,
        successUrl:
          base +
          '/advertising?opportunity-page-partner=success&session_id={CHECKOUT_SESSION_ID}',
        cancelUrl: base + '/advertising?opportunity-page-partner=cancelled',
      });

const { sessionFromRequest, json, setCors } = require('../auth');
const { getSupabaseAdmin, isSupabaseConfigured } = require('../supabase');
const { enforceRateLimit } = require('../rate-limit');

const REASONS = new Set([
  'fake_or_paid',
  'not_attendee',
  'misleading',
  'offensive',
  'spam',
  'other',
]);

function parseBody(req) {
  let body = req.body;
  if (typeof body === 'string') {
    try {
      body = JSON.parse(body);
    } catch {
      body = {};
    }
  }
  return body || {};
}

module.exports = async function handler(req, res) {
  setCors(req, res);
  res.setHeader('Cache-Control', 'no-store');

  if (req.method !== 'POST') return json(res, 405, { error: 'method_not_allowed' });

  const limited = enforceRateLimit(req, res, 'report_review', {
    max: 10,
    windowMs: 600_000,
  });
  if (!limited.allowed) {
    return json(res, 429, {
      ok: false,
      error: 'rate_limited',
      message: 'Too many reports. Please wait a few minutes and try again.',
      retryAfterSec: limited.retryAfterSec,
    });
  }

  if (!isSupabaseConfigured()) {
    return json(res, 503, { ok: false, error: 'not_configured' });
  }

  const body = parseBody(req);
  const reviewId = String(body.review_id || body.reviewId || '').trim();
  const reason = String(body.reason || '').trim().toLowerCase();
  const details = String(body.details || '').trim().slice(0, 2000);
  const organiserId = body.organiser_id || body.organiserId || null;
  const eventId = body.event_id || body.eventId || null;
  const reviewSnippet = String(body.review_snippet || body.reviewSnippet || '').trim().slice(0, 500);

  if (!reviewId) {
    return json(res, 400, { ok: false, error: 'review_id_required' });
  }
  if (!REASONS.has(reason)) {
    return json(res, 400, { ok: false, error: 'invalid_reason' });
  }

  const session = sessionFromRequest(req);
  const reporterEmail = session?.email ? String(session.email).trim() : null;

  try {
    const sb = getSupabaseAdmin();
    const insertRes = await sb
      .from('review_reports')
      .insert({
        review_id: reviewId,
        organiser_id: organiserId,
        event_id: eventId,
        review_snippet: reviewSnippet,
        reporter_user_id: session?.userId || session?.sub || null,
        reporter_email: reporterEmail,
        reason,
        details: details || null,
        status: 'open',
      })
      .select('id, created_at')
      .single();
    if (insertRes.error) throw new Error(insertRes.error.message);
    return json(res, 200, { ok: true, id: insertRes.data.id, createdAt: insertRes.data.created_at });
  } catch (e) {
    return json(res, 500, { ok: false, error: 'report_failed', message: e.message });
  }
};

...
(function () {
  ...
  function syncSpotlightLoopScroll() {
    ...
    sc.syncLoopScroll(track, loopWidth, featured.length, '.premium-card');
  }
  ...
  function advanceSpotlight(dir) {
    ...
    if (looping && sc) {
      var loopWidth =
        ...
        sc.measureLoopWidth(track, featured.length, '.premium-card');
      if (
        !sc.advanceLoop(track, dir, step, behavior, loopWidth, featured.length, '.premium-card')
      ) {

/**
 * Export 52 networking groups count-up as MP4.
 * Usage: node export-52-groups-ad.mjs
 */
...
const size = 1080;
const htmlPath = path.join(__dirname, 'linkedin-ad-52-groups.html');
const outDir = path.join(__dirname, 'exports');
const outName = 'rosie-52-groups-countup.mp4';
...
const durationMs = await page.evaluate(() => window.__adDurationMs || 11000);
...
const rawMp4 = path.join(outDir, '52-groups-raw.mp4');
...
try { fs.unlinkSync(webmFinal); } catch (_) {}
...
// Still endcard matching the 92 graphic, for Buffer thumbnail / fallback

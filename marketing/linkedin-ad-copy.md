...
Square PDFs for LinkedIn document posts, carousels, or DMs. All hosted at `/assets/guides/` and on `/guides`.
...
```bash
cd marketing
node export-linkedin-pdfs.mjs
node export-listing-pdf.mjs   # also regenerates A4 how-to-list
```
...
## Organiser benefits (LinkedIn post copy)
...
## Why attendees should browse here (LinkedIn post copy)
...
```bash
cd marketing
node export-opportunity-ad.mjs
node export-organiser-ad.mjs
node export-ad-video.mjs attendee dark
```

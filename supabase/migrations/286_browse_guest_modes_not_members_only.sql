-- Guest programme / membership-after-visits keep a members_only tier for after
-- complimentary visits, but newcomers can still book guest passes — never flag
-- those listings as members_only_event on browse.
...
select
  ...
  (
    coalesce(e.attendance_mode, 'tickets') not in (
...
comment on view public.browse_events_index is
  'Public browse catalogue — CE / guest programme / membership meetings are never members_only_event.';

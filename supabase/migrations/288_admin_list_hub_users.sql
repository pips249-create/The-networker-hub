-- Fast admin user directory: filter + page in Postgres (joins auth.users).
-- Avoids serverless timeouts from auth.admin.listUsers + full-table scans.
...
revoke all on function public.admin_list_hub_users(text, text, integer, integer) from public;
grant execute on function public.admin_list_hub_users(text, text, integer, integer) to service_role;

comment on function public.admin_list_hub_users(text, text, integer, integer) is
  'Admin Command Center user directory — server-side search and pagination over hub_accounts + auth.users.';

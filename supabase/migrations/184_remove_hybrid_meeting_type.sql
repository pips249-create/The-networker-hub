-- Remove Hybrid as a meeting format: backfill existing rows and tighten constraints.

update public.events
set meeting_type = 'Online'
where coalesce(meeting_type, '') ilike '%hybrid%'
  and coalesce(trim(meeting_link), '') <> '';

update public.events
set meeting_type = 'In person'
where coalesce(meeting_type, '') ilike '%hybrid%';

alter table public.events drop constraint if exists events_meeting_type_check;

alter table public.events
  add constraint events_meeting_type_check
  check (meeting_type in ('In person', 'Online'));

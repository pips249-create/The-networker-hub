#!/usr/bin/env node
/**
 * Send Franchise Email 2 (claim follow-up) via Resend.
 *
 * Audience CSV must come from scripts/build-franchise-claim-followup.js
 *
 * Usage:
 *   node scripts/send-franchise-claim-followup.js                         # dry-run
 *   node scripts/send-franchise-claim-followup.js --test you@example.com
 *   node scripts/send-franchise-claim-followup.js --send --limit=50
 *   node scripts/send-franchise-claim-followup.js --send --offset=50 --limit=50
 */
const fs = require('fs');
  ...
  if (!fs.existsSync(file)) {
    throw new Error(
      'Missing ' +
        ...
        ' — run: node scripts/build-franchise-claim-followup.js'
    );
  }
...
async function sendOne(row, toOverride) {
  ...
  return sendTemplatedEmail({
    ...
    resendTags: [
      ...
      { name: 'segment', value: 'franchise_unclaimed_followup' },
    ],
  });
}
...
(async () => {
  ...
  console.log('Audience: Email 1 recipients still unclaimed / unpaid');
  ...
  if (!doSend) {
    ...
    if (slice.length > 5) console.log(' ... and', slice.length > 5 ? slice.length - 5 : 0, 'more');
    return;
  }

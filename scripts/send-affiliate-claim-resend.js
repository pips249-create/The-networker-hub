#!/usr/bin/env node
/**
 * Send affiliate-only opportunity claim invites via Resend.
 *
 * Audience CSV must come from scripts/build-affiliate-claim-resend.js
 *
 * Usage:
 *   node scripts/send-affiliate-claim-resend.js                         # dry-run
 *   node scripts/send-affiliate-claim-resend.js --test you@example.com
 *   node scripts/send-affiliate-claim-resend.js --send --limit=10
 *   node scripts/send-affiliate-claim-resend.js --send
 */
const fs = require('fs');
...
const CSV = path.join(root, 'data/Affiliate-Claim-Resend.csv');
const SLUG = 'affiliate_claim_invite';
const REPLY_TO = 'catherine@thenetworkeruk.com';
const SUBJECT = '{{opportunity_title}} — claim your affiliate programme listing';
  ...
  if (!fs.existsSync(file)) {
    throw new Error(
      'Missing ' +
        ...
        ' — run: node scripts/build-affiliate-claim-resend.js'
    );
  }
...
async function sendOne(row, toOverride) {
  ...
  const title = row.title || 'your affiliate programme';
  ...
  return sendTemplatedEmail({
    ...
    replyTo: REPLY_TO,
    resendTags: [
      { name: 'campaign', value: 'opportunity_claim_affiliate' },
      { name: 'segment', value: 'affiliate_unclaimed' },
    ],
  });
}
...
(async () => {
  ...
  console.log('Audience: unclaimed affiliate listings only');

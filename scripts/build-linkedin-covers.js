...
function commonBg(accent, theme) {
  const isOpp = theme === 'opportunity';
  const stops = isOpp
    ? `      <stop offset="0%" stop-color="#0d1f3c"/>
      <stop offset="55%" stop-color="#162847"/>
  ...
  const orbs = isOpp
    ? `  <circle cx="1520" cy="40" r="180" fill="#c9961f" opacity="0.18"/>
  <circle cx="1460" cy="380" r="140" fill="#e8b84b" opacity="0.12"/>`
...
function hubCredit(logoSrc, opts) {
  ...
  const isOpp = opts && opts.theme === 'opportunity';
  ...
  const fill = isOpp ? '#8d99ae' : '#5c5557';
  return `
  <image href="${logoSrc}" xlink:href="${logoSrc}" x="${x}" y="${y - 18}" width="${logoW}" height="${logoH}" preserveAspectRatio="xMidYMid meet" opacity="0.92"/>
  <text x="${x + logoW / 2}" y="${y + logoH + 6}" text-anchor="middle" fill="${fill}" font-family="Arial, Helvetica, sans-serif" font-size="11">${label}</text>`;
}

function organiserBody(c) {
  const isOpp = c.theme === 'opportunity';
  const kicker = isOpp ? '#e8b84b' : '#9a7aa8';
  const title = isOpp ? '#ffffff' : '#4a4446';
  const sub = isOpp ? '#8d99ae' : '#5c5557';
  // Copy starts mid-banner so LinkedIn avatar (left) never covers it
  return `  <g transform="translate(520, 108)">
    <text x="0" y="0" fill="${kicker}" font-family="Arial, Helvetica, sans-serif" font-size="16" font-weight="700" letter-spacing="2.4">${c.kicker}</text>
    <text x="0" y="58" fill="${title}" font-family="Georgia, 'Times New Roman', serif" font-size="46">${c.line1}</text>
    <text x="0" y="116" fill="${title}" font-family="Georgia, 'Times New Roman', serif" font-size="46">${c.line2}</text>
    <text x="0" y="168" fill="${sub}" font-family="Arial, Helvetica, sans-serif" font-size="20">${c.line3}</text>
  </g>`;
}
...
${commonBg(c.accent, c.theme)}
${organiserBody(c)}
${hubCredit(logoSrc, { hubEmphasis: Boolean(c.hubEmphasis), theme: c.theme })}
</svg>`;
}
